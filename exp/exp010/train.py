...
EXP_NAME = "exp010_add_inital_weight"
...
def set_initial_weights(
    ...
    old_tokenizer = AutoTokenizer.from_pretrained(model_name, trust_remote_code=True)

    for all_completion in all_completions:
        new_token_id = new_tokenizer.convert_tokens_to_ids(all_completion)

        # 特殊トークン追加前のトークナイズ結果を得る
        sub_tokens = old_tokenizer.tokenize(all_completion, add_special_tokens=False)
        sub_token_ids = old_tokenizer.convert_tokens_to_ids(sub_tokens)

        # サブトークン埋め込みの平均で初期化
        with torch.no_grad():
            old_embeds = model.get_input_embeddings().weight[sub_token_ids]
            avg_embed = old_embeds.mean(dim=0)
            model.get_input_embeddings().weight[new_token_id] = avg_embed
...
if __name__ == "__main__":
    ...
    # 特殊トークンの重みを、元のトークンの平均にする。
    model = set_initial_weights(model, tokenizer, MODEL_NAME, all_completions)
