...
USE_FOLD = 0
TEACHER_MODEL_NAME = "outputs/exp034/20251005222228/checkpoint/checkpoint-6117-cp"  # TODO: exp036に差し替える
KD_ALPHA = 0.5   # 蒸留の寄与（0〜1）
KD_T = 2.0       # 温度
...
class KDTrainer(SFTTrainer):
    def __init__(self, *args, teacher_model=None, kd_alpha=0.5, kd_t=2.0, **kwargs):
        super().__init__(*args, **kwargs)
        assert teacher_model is not None, "teacher_model is required for KD."
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        if target_mask.any():
            with torch.no_grad():
                ...
                teacher_logits = t_out.logits  # (B, L, V)

            T = self.kd_t
            s_logp = F.log_softmax(student_logits / T, dim=-1)  # (B,L,V)
            t_p    = F.softmax(teacher_logits / T, dim=-1)      # (B,L,V)

            sel = target_mask.unsqueeze(-1).expand_as(s_logp)   # (B,L,V)
            s_logp_sel = s_logp[sel].view(-1, s_logp.size(-1))  # (N,V)
            t_p_sel    = t_p[sel].view(-1, t_p.size(-1))        # (N,V)

            kd_loss = F.kl_div(s_logp_sel, t_p_sel, reduction="batchmean") * (T * T)

        # --- (C) 合算 ---
        loss = (1.0 - self.kd_alpha) * ce_loss + self.kd_alpha * kd_loss
